...
def check_prime(number):
    if number >= 2:
        # Going through potential divisors
        for i in range(2, number):
           
            if number % i == 0:
                print(f"{number} is not a prime number.")
                return # Stops the function immediately if a divisor is found
        
        # Else if the loop finished without executing return, it must be prime
